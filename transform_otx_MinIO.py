...
def build_processed_parquet_object_name(processed_dt: datetime) -> str:
    ...
    return (
        f"{OTX_PROCESSED_PREFIX}"
        f"year={year}/month={month}/day={day}/"
        f"otx_processed_{timestamp}.parquet"
    )
...
def main():
    ...
    latest_raw_object = get_latest_object_name(client, MINIO_BUCKET, OTX_RAW_PREFIX)
    if not latest_raw_object:
        raise SystemExit(f"No OTX raw file found in MinIO under prefix: {OTX_RAW_PREFIX}")
    ...
    for record in records:
        processed_record = {
            ...
            "otx_reputation": record.get("otx_reputation"),
            "otx_pulse_count": record.get("pulse_count", 0),
            "otx_pulse_names": record.get("pulse_names", []) or [],
            "otx_passive_dns_count": record.get("passive_dns_count", 0),
            "otx_general_status": record.get("general_status"),
            "otx_passive_dns_status": record.get("passive_dns_status"),

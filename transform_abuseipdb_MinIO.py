...
def build_processed_parquet_object_name(processed_dt: datetime) -> str:
    ...
    return (
        f"{ABUSE_PROCESSED_PREFIX}"
        f"year={year}/month={month}/day={day}/"
        f"abuseipdb_processed_{timestamp}.parquet"
    )
...
def main():
    ...
    latest_raw_object = get_latest_object_name(client, MINIO_BUCKET, ABUSE_RAW_PREFIX)
    if not latest_raw_object:
        raise SystemExit(f"No AbuseIPDB raw file found in MinIO under prefix: {ABUSE_RAW_PREFIX}")
    ...
    raw_records = raw_payload.get("data", []) if isinstance(raw_payload, dict) else []
    raw_ingestion_time = raw_payload.get("ingestion_time") if isinstance(raw_payload, dict) else None
    ...
    for record in raw_records:
        processed_record = {
            ...
            "ingestion_time": raw_ingestion_time,
